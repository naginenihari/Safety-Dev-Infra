resource "aws_cloudfront_distribution" "safety" {
  origin {
    # safety-dev.naginenihariaws.store
    ...
    custom_origin_config {
      http_port              = 80
      https_port             = 443
      origin_protocol_policy = "https-only" # or "https-only" or "match-viewer"
      origin_ssl_protocols   = ["TLSv1.2"]
    }
  }
enabled             = true
 #dev.naginenihariaws.store
...
resource "aws_route53_record" "cdn" {
  ...
  alias {
    ...
    name                   = aws_cloudfront_distribution.safety.domain_name
    zone_id                = aws_cloudfront_distribution.safety.hosted_zone_id
